...
首先，最基本的来说， BlockingQueue 是一个**先进先出**的队列（Queue），为什么说是阻塞（Blocking）的呢？是因为 BlockingQueue 支持当获取队列元素但是队列为空时，会阻塞等待队列中有元素再返回；也支持添加元素时，如果队列已满，那么等到队列可以放入新元素时再放入。下图是对这个原理的阐述：

![img](http://blog-1259650185.cosbj.myqcloud.com/img/202201/04/1641281468.png)
...
最后，BlockingQueue 在生产者-消费者的场景中，是**支持多消费者和多生产者**的，说的其实就是线程安全问题。

相信上面说的每一句都很清楚了，BlockingQueue 是一个比较简单的线程安全容器，下面我会分析其具体的在 JDK 中的实现。

## 3. BlockingQueue 实现之 ArrayBlockingQueue

ArrayBlockingQueue 是 BlockingQueue 接口的有界队列实现类，底层采用数组来实现，并发控制采用可重入锁来控制，不管是插入操作还是读取操作，都需要获取到锁才能进行操作，具体地说，它采用一个 ReentrantLock 和相应的两个 Condition 来实现。
...
<img src="http://blog-1259650185.cosbj.myqcloud.com/img/202201/04/1641263958.png" alt="array-blocking-queue" style="zoom:50%;" />
...
## 4. BlockingQueue 实现之 LinkedBlockingQueue
...
<img src="https://www.javadoop.com/blogimages/java-concurrent-queue/linked-blocking-queue.png" alt="linked-blocking-queue" style="zoom:50%;" />
...
## 5. BlockingQueue 实现之 SynchronousQueue
...
![synchronous-queue](http://blog-1259650185.cosbj.myqcloud.com/img/202201/04/1641266037.png)
...
Object awaitFulfill(QNode s, E e, boolean timed, long nanos) {
    ...
    for (;;) {
        ...
        // spinForTimeoutThreshold ，剩余时间小于这个阈值的时候，就不要进行挂起了，自旋的性能会比较好
...
## 6. BlockingQueue 实现之 PriorityBlockingQueue
...
<img src="http://blog-1259650185.cosbj.myqcloud.com/img/202201/04/1641279844.png" alt="priority-blocking-queue-1" style="zoom:50%;" />
...
我们用图来示意一下，我们接下来要将 **11** 插入到队列中，看看 siftUp（节点上浮）是怎么操作的。

<img src="http://blog-1259650185.cosbj.myqcloud.com/img/202201/04/1641280800.png" alt="priority-blocking-queue-2" style="zoom:50%;" />
...
废话不多说，出队是非常简单的，因为队头就是最小的元素，对应的是数组的第一个元素。难点是队头出队后，需要调整树（节点下沉）。
...
![priority-blocking-queue-3](http://blog-1259650185.cosbj.myqcloud.com/img/202201/04/1641281403.png)
...
![priority-blocking-queue-4](http://blog-1259650185.cosbj.myqcloud.com/img/202201/04/1641281432.png)
